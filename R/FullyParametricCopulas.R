...
#' @title Estimation dependent censoring model.
...
ParamCop<-function(Y,Delta,Copula,Dist.T,Dist.C){
  ...
  if(!(Copula %in% c("frank","gumbel","clayton","gaussian","indep"))){
    ...
    return(NA)
  }else if( (Copula == "gumbel" | Copula == "clayton") & (Dist.T == "llogis" | Dist.C == "llogis")){
    print("ERROR : log-logistic marginals cannot be specified with the gumbel or clayton copula")
    return(NA)
  }else if( (Copula == "gumbel" | Copula == "clayton") & (Dist.T != Dist.C)){
    print("ERROR : marginals need to be both lnorm or both weibull")
    return(NA)
  }else if( (Copula == "gaussian") & (Dist.T == "llogis" | Dist.C == "llogis" | Dist.T == "weibull"| Dist.C == "weibull")){
    print("ERROR : The gaussian copula requires log-normal marginals")
    return(NA)
  } else {
